...
-- Create the widget
dashboard =
    wibox({ visible = false, ontop = true, type = "dock", screen = screen.primary })
awful.placement.maximize(dashboard)

dashboard.bg = beautiful.dashboard_bg
    or beautiful.exit_screen_bg
    or beautiful.wibar_bg
    or "#111111"
...
awesome.connect_signal("evil::disk", function(used, total)
    ...
    disk_hover_text_value.markup =
        helpers.colorize_text(tostring(helpers.round(total - used, 1)) .. "G", x.color4)
end)
...
awesome.connect_signal(
    "evil::coronavirus",
    function(cases_total, cases_today, deaths_total, deaths_today)
...
corona_box:buttons(gears.table.join(
    ...
    awful.button({}, 1, function()
        awful.spawn.with_shell(
            user.browser .. " https://www.worldometers.info/coronavirus/"
        )
        dashboard_hide()
    end)
))
...
local petal_top_right = create_url_petal(
    "YT",
    x.color1,
    x.color9,
    "https://youtube.com/",
...
local petal_bottom_right = create_url_petal(
    "4C",
    x.color2,
    x.color10,
    "https://4chan.org/",
...
local petal_bottom_left = create_url_petal(
    "RD",
    x.color3,
    x.color11,
    "https://reddit.com/",
...
local function update_notification_state_icon()
    if naughty.suspended then
        notification_state.markup =
            helpers.colorize_text(notification_state.text, x.color8)
    else
        notification_state.markup =
            helpers.colorize_text(notification_state.text, x.color2)
    end
end
update_notification_state_icon()
local notification_state_box =
    create_boxed_widget(notification_state, dpi(150), dpi(78), x.background)
