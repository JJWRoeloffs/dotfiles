...
awesome.connect_signal("evil::battery", function(battery)
    ...
    if not charger_plugged then
        ...
        if
            battery <= user.battery_threshold_critical
            and not battery_critical_already_notified
        then
            ...
            timeout = 0
        elseif
            battery <= user.battery_threshold_low and not battery_low_already_notified
        then
