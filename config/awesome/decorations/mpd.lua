...
volume_bar:connect_signal("button::press", function(_, lx, __, button)
    if button == 1 then
        awful.spawn.with_shell(
            "mpc volume " .. tostring(math.ceil(lx * 100 / volume_bar.forced_width))
        )
    end
end)
...
local mpd_create_decoration = function(c)
    -- Main titlebar
    awful
        .titlebar(c, { position = "top", size = main_titlebar_size, bg = x.background })
        :setup({
            {
                {
    ...
    awful
        .titlebar(c, { position = toolbar_position, size = toolbar_size, bg = x.background })
        :setup({
            {
                {
