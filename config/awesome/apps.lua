...
apps.weechat = function()
    helpers.run_or_raise(
        { instance = "weechat" },
        true,
        user.terminal .. " --class weechat -e weechat"
    )
end
apps.mail = function()
    helpers.run_or_raise(
...
apps.torrent = function()
    helpers.run_or_raise(
        { instance = "torrent" },
        true,
        user.terminal .. " --class torrent -e transmission-remote-cli"
    )
end
...
apps.night_mode = function()
    local cmd =
        "pgrep redshift > /dev/null && (pkill redshift && echo 'OFF') || (echo 'ON' && redshift)"
...
apps.process_monitor = function()
    helpers.run_or_raise(
        { instance = "htop" },
        false,
        user.terminal .. " --class htop -e htop",
        { switchtotag = true }
    )
end
...
apps.markdown_input = function()
    helpers.scratchpad(
        ...
        user.terminal
            .. " --class markdown_input -e nvim -c 'startinsert' /tmp/scratchpad.md",
        nil
    )
end
...
function apps.screenshot(action, delay)
    ...
    elseif action == "gimp" then
        awful.spawn.with_shell(
            "cd " .. user.dirs.screenshots .. " && gimp $(ls -t | head -n1)"
        )
    ...
    screenshot_copy:connect_signal("invoked", function()
        awful.spawn.with_shell(
            "xclip -selection clipboard -t image/png " .. filename .. " &>/dev/null"
        )
    end)
